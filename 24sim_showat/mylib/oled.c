...
void OLED_ShowUploading(u8 x, u8 y)
{      			    
	u8 t, i, adder = 0, flag = 0;
	...
	for(i = 0; i < 8; i++)
	{
		...
		for(t = 0; t < 16; t++)
		{
			OLED_WR_Byte(upload[i][t], OLED_DATA);
			adder += 1;
		}
	}
}
...
void OLED_ShowDownloading(u8 x, u8 y)
{      			    
	u8 t, i, adder = 0, flag = 0;
	...
	for(i = 0; i < 8; i++)
	{
		...
		for(t = 0; t < 16; t++)
		{
			OLED_WR_Byte(download[i][t], OLED_DATA);
			adder += 1;
		}
	}
}

void OLED_ShowLinking(u8 x, u8 y)
{      			    
	u8 t, i, adder = 0, flag = 0;
	...
	for(i = 0; i < 8; i++)
	{
		...
		for(t = 0; t < 16; t++)
		{
			OLED_WR_Byte(link[i][t], OLED_DATA);
			adder += 1;
		}
	}
}
...
/********************************************
// Oled Clear Screen  page 6-7
********************************************/
void OLED_Clear_status(void)  
{  
	u8 i, n;
	for(i = 6; i < 8; i++)  
	{  
